...
and exact-path NOPASSWD sudo; invoke them as `sudo -n /abs/path/e2e/<script>.sh`.
...
`full_matrix.sh` runs relay (one and four carriers), the non-sudo fault bank,
privileged filesystem/transport/bandwidth, and MinIO.
