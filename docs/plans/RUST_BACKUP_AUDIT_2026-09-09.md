...
| DEAD-3 | `CheckConfig::role` (with its `CheckRole` enum) was set by the caller from the direct-path role and never read: both peers run the same symmetric probe, each sending to every candidate and answering whatever arrives. Config that advertises an asymmetry the code does not have is worse than no config, so the field is gone and the symmetry is stated where the struct is defined |
| BOUND-1 | The learned-address cache was documented as bounded and was not: entries expire only when the key they belong to is recalled, so a process that opens many channels and never revisits them kept one entry per channel for its whole life. Capped at 128 with expiry swept on insert and the least recently learned entry evicted — the cache is advisory, so an eviction costs a probe and never a connection. The two tests that share this process-global cache are now serialised, because the filling test would otherwise evict the other's key between its `remember` and its `recall` |
| DOC-5 | `adaptive_nat::plan_for` — the punch pacing and check-window builder — is implemented and unit-tested, and nothing calls it: only `classify_nat` is wired, to fill the profile sent to the peer. That is exactly what plan row F6.2 leaves open, but `docs/TRANSPORT.md` did not say so, so a reader could reasonably assume punch timing already adapts to the observed NAT class. Stated explicitly |
| DEAD-4 | bore's registration channel for *additional* provider connections (`PendingCarriers`, `TokenGuard`) was carried over and never used: a `DashMap` allocated per server, threaded through three handlers and then ignored by the one that received it. In this design a provider registers once and every data carrier is a substream on that one mux, so the mechanism has no meaning here — and an unused mechanism reads as a supported one. Removing it also removed the `clippy::too_many_arguments` allow that its parameter had made necessary, leaving the whole workspace with exactly one `#[allow]`, inside a test module |
| DEAD-5 | `rb-transport::prefixed` — bore's prefix-replay stream wrapper — had no caller anywhere: bore uses it to tell HTTP from bore on a shared port, and this server never sniffs. Removed, with the port-mapping row in the V1 plan updated so the mapping stays truthful; `vendored-from-bore/prefixed.rs` still holds the original. The crate doc also still said the direct UDP/QUIC path was "deferred to Phase 1", which has been shipped for two phases |
